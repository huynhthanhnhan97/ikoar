...
class eventController {
    ...
    timeHold = 10; // When enough this time, mouse hold will be true
    ...
    constructor(rendererDomElement, arScene, isPortrait, objectRoot, hint_Content) {
    ...
    setPickPosition = (event) => {
        this.pickPosition.x = (((event.clientX - this.rendererDomElement.getBoundingClientRect().left / 1.5) / this.rendererDomElement.getBoundingClientRect().width / 1.5) * -2 + 1);
        this.pickPosition.y = (((event.clientY - this.rendererDomElement.getBoundingClientRect().top) / this.rendererDomElement.getBoundingClientRect().height / 1.5) * -2 + 1); // note we flip Y
        if (this.isPortrait) {
    ...
    attachEvent = () => {
        ...
        this.rendererDomElement.addEventListener('mouseup', (ev) => {
            ...
            this.clearPickPosition();
            this.arScene.setContext(this.hint_Content);
        }, false);
        ...
        this.rendererDomElement.addEventListener('touchstart', (ev) => {
            this.arScene.setUnboxFlag(true);
            if (this.arScene.isInteract()) {
                this.countTimeNotInteract = new Date().getSeconds();
                ...
                if (ev.touches.length > 1) {
                    ...
                    if (this.isRotate) {
                        ...
                        if (this.preMousePos.x && this.preMousePos.y) {
                            ...
                            console.log(this.objectRoot.rotation.y, this.objectRoot.rotation.x)
                            this.objectRoot.translateZ(-this.modelCenterZ);
                        }
